 ...
 * @author Dawid
 */
class Kaliop_Logger_Model_Observer extends Mage_Core_Model_Abstract {
    ...
    public function customerLogEvent(Varien_Event_Observer $observer) {
        $configEnabled = Mage::getStoreConfig('kaliop_section/general/logger_enabled');
        if ($configEnabled) {
            $customerData = $observer->getCustomer()->getData();
            $customerName = $customerData['firstname'] . " " . $customerData['lastname'];
            $eventName = substr(strrchr($observer->getEvent()->getName(), "_"), 1);
            $model = Mage::getModel('kaliop_logger/logger');
